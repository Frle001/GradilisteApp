...
func main() {
	...
	caRepo := repositories.NewCompanyAssetsRepository(db)
	caSvc := services.NewCompanyAssetsService(caRepo)
	caHandler := handlers.NewCompanyAssetsHandler(caSvc)

	empDocRepo := repositories.NewDocumentationRepository(db)
	empDocSvc := services.NewDocumentationService(empDocRepo, storageSvc)
	empDocHandler := handlers.NewDocumentationHandler(empDocSvc)
